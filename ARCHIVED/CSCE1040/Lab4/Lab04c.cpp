...
int ascii_totals(char* word, int str_len);
int num_of_t(int &t_count);
...
{ //***MAIN***
   ...
   ***************************************************************/
  printf("+----------------------------------------------------+\n");
  printf("|  CSCE1040 -- Computer Science II                   |\n");
  printf("|  NAME:  Pat Burke               EUID:pwb0013       |\n");
  printf("|  EMAIL: Patrick.Burke@unt.edu                      |\n");
  printf("|  Lab Assignment 4               Spring 2016        |\n");
  printf("+----------------------------------------------------+\n\n");
  ...
  while (wordNum < MAX_WORDS)
    {     //***WHILE wordNum < MAX_WORDS***
 
    scanf("%d",&str_len);  //retrieve the string length of the next word
    asciiTot = 0;  // reset the ascii-value total for the next word
    t_count = 0;   // reset the "count of t characters" for the next word

    for (i=0; i<=str_len-1 ;++i)
      {     // FOR i to str_len-1
      scanf("%c",&word[i]);           //get the next character
	}


      asciiTot = ascii_totals(word, str_len);
      t_count = num_of_t(t_count);

    printf("%-14s %-1d  %5d \n",word,t_count,asciiTot);
    wordNum = wordNum + 1;
