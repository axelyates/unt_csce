...
int main()
{
	...
	char saved_file[31];				//variable that references a saved file
	...
	printf("9.  Load Grade Book (from a user-specified file)\n");		//user selects if they want to load a gradebook from a file
	printf("10. Store Grade Book (to a user-specified file)\n");		//user selects if they want to save current gradebook
	printf("11. Exit\n");							//user selects if they want to end the program
	while(selection != 11){							//while loop that continually asks user for input until they choose 9 to exit
	...
	switch(selection)							//beginning of switch statement
	{
		...
		case 9:								//runs if variable "selecction" is equal to 9
			case9(courses, students, num_of_courses, num_of_students, saved_file);

		break;
		
		case 10:							//runs if variable "selecction" is equal to 10
			case10(courses, students, num_of_courses, num_of_students, saved_file);

		break;

		case 11:							//runs if variable "selecction" is equal to 11
